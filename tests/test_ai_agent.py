...
async def test_health_check_healthy(ai_agent):
    ...
    with patch.object(
        ai_agent.client, "list", return_value=mock_list_response
    ):
...
async def test_health_check_timeout(ai_agent):
    ...
    with patch.object(
        ai_agent.client, "list", side_effect=asyncio.TimeoutError
    ):
        is_healthy = await ai_agent.health_check()
        assert is_healthy is False
...
async def test_analyze_sensor_state_success(ai_agent):
    ...
    with patch.object(
        ai_agent.client, "generate", return_value=mock_llm_response
    ):
        sensor_data = {
            "soil_moisture": 65.5,
            "ambient_light": 450,
...
async def test_analyze_sensor_state_timeout(ai_agent):
    ...
    with patch.object(
        ai_agent.client, "generate", side_effect=asyncio.TimeoutError
    ):
